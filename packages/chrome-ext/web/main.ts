...
async function addSidepanelToggleButton() {
  const toolbarRightEle = await while$('ms-toolbar .toolbar-right')
  const buttonEle = document.createElement("button")
  buttonEle.className = 'mat-mdc-tooltip-trigger ms-button-borderless ms-button-icon'
  const html = String.raw
  buttonEle.innerHTML = html`<img src="${chrome.runtime.getURL('icons/icon128.png')}"/>`

  toolbarRightEle.insertBefore(buttonEle, toolbarRightEle.firstElementChild)
}

try {
  connectToBackground();
  addSidepanelToggleButton()
} catch (error) {
