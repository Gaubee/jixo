...
import { LogFileSchema, type JobInfoData } from "../entities.js";
import { getLogFilePath, ensureJixoDirsExist } from "./internal.js";
...
class WorkspaceManager {
    ...
    async createJob(jobName: string, jobGoal: string): Promise<LogManager> {
        const jobInfo: JobInfoData = {
            jobGoal,
            workDir: this.workspaceDir,
        };
        // createIsolated handles file creation if it doesn't exist.
        return logManagerFactory.createIsolated(jobName, jobInfo);
    }
}
